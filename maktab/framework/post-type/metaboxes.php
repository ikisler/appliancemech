    ...
    function settings_file($settings){ ?>
        <tr id="<?php echo $settings['id']; ?>">
            ...
            <td>
                ...
                <a href="#" class="button insert-images theme_button format" onclick="browseimage('<?php echo $settings['id']; ?>')"><?php _e('Insert image', "otouch"); ?></a>
            </td>
        </tr><?php
    }
}
...
if (!function_exists('settings_gallery')) {
    function settings_gallery($settings){
        ...
        $gallery_thumbs = '';
        $button_text = ($meta) ? __('Edit Gallery', 'otouch') : __('Upload Images', 'otouch');
        if( $meta ) {
            $thumbs = explode(',', $meta);
                ...
                <?php $menus = get_terms( 'nav_menu', array( 'hide_empty' => false ) );
                        if ( !$menus ) {
                            ...
                            echo '<select name="'.$settings['id'].'" data-value="'.$settings['value'].'">';
									echo '<option value="">'. __('Default', 'otouch	') . '</option>';
